...
// Joysticks
  ...
  private void configureButtonBindings() {
    ...
    new Trigger(operateCommandJoystick.button(A)).onTrue((intakeRollerSubsystem.intakeNoteCommand().raceWith(transferSubsystem.getFromIntake())).until(operateCommandJoystick.button(B)));
    // new Trigger(operateCommandJoystick.button(X)).onTrue(closeIntakeCommand());
    // new Trigger(operateCommandJoystick.button(X)).onTrue(shootingArmSubsystem.OpenForIntakeCommand());
    ...
    new Trigger(operateCommandJoystick.button(LB)).onTrue(shootingSubsystem.spinWheelsToSpeedCommand(SHOOT_AMP_SPEED).raceWith(transferSubsystem.amp()));
    ...
    // new Trigger(operateCommandJoystick.povLeft()).onTrue(intakeLiftSubsystem.goToCommand(FOLD)); 
    // new Trigger(operateCommandJoystick.povDown()).onTrue(intakeLiftSubsystem.goToCommand(GROUND)); 
    
    // new Trigger(operateCommandJoystick.povLeft()).onTrue(shootingArmSubsystem.testtt(SUB_INTAKE_POS)); 
    // new Trigger(operateCommandJoystick.povRight()).onTrue(shootingArmSubsystem.testtt(SHOOT_AMP_POS)); 
    // new Trigger(operateCommandJoystick.povUp()).onTrue(shootingArmSubsystem.OpenSlow()); 

    


    // new Trigger(driverCommandJoystick.button(A)).onTrue(new TurnByDegreeCommand(driveSubsystem, 10.0));
